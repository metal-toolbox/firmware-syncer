...
//
// expects a downloader initialized with tmpDir directory as the base directory
// returns nil if verify was successful
func VerifyFilestoreFile(ctx context.Context, filePath, fileSHA256, tmpDir string, downloader *Downloader, signer *Signer, logger *logrus.Logger) error {
	// create local tmp directory
	tmpDir, err := os.MkdirTemp(tmpDir, "binfiles-")
	if err != nil {
		return errors.Wrap(ErrRemoteVerifyFail, err.Error())
	}

	srcPathDir := path.Dir(filePath)
	...
	// bin
	binFile := path.Base(filePath)
	// bin.SHA256
	checksumFile := binFile + SumSuffix
	// bin.SHA256.sig
	sigFile := checksumFile + SigSuffix

	// Download repo file to tmp directory - the local tmp directory is mounted with /tmp
	// hence here we pass the relative path asda/{bin, bin.SHA256, bin.SHA256.sig}
	for _, file := range []string{binFile, checksumFile, sigFile} {
		// upload the checksum and signature tmp files to the dst directory
		// downloader.tmp is mounted with /tmp as the root, hence this mangling (TODO: consider mounting / ?)
		tmpFileRelPath := path.Join(tmpDirBase, file)
		// downloader.filestore need to be given the full path including the filename or we end up clobbering a directory
		srcFileRelPath := path.Join(srcPathDir, file)

		logger.WithFields(logrus.Fields{"tmp": path.Join(tmpDir, tmpFileRelPath), "src": downloader.FilestoreURL() + filePath}).Trace("download file to verify checksum, signature")

		err = downloader.CopyFilestoreToLocalTmp(ctx, tmpFileRelPath, srcFileRelPath)
		if err != nil {
			return errors.Wrap(ErrDownloadVerify, err.Error())
		}
	}
	...
	// absolute path to the sig, checksum file
	tmpSigFile := path.Join(tmpDir, filepath.Base(sigFile))
	tmpChecksumFile := path.Join(tmpDir, filepath.Base(checksumFile))
	...
	err = SHA256ChecksumValidate(tmpChecksumFile, fileSHA256)
	if err != nil {
		return err
	}
